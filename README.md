...
## Fixed: images not showing on GitHub Pages

If you deployed to GitHub Pages and saw the page load with no images, this was the cause: your
repo is served from `github.io/professional-portfolio/`, not the domain root, so every
hardcoded `/assets/...` path was missing that prefix. Fixed by:

- `next.config.mjs` now sets `output: "export"` (produces a plain static `out/` folder, which
  is what GitHub Pages needs), plus `basePath: "/professional-portfolio"` and a matching
  `assetPrefix`.
- **If you ever rename the GitHub repo**, update `REPO_NAME` at the top of
  `next.config.mjs` to match, or images will break again.
- Added `lib/basePath.ts`, which `components/ProjectImage.tsx` and `components/Hero.tsx` now
  use to prepend that prefix to their image/PDF paths. (Next's own `<Link>` component already
  handled this automatically for navigation — only raw `<img>`/`<a>` tags needed the fix.)
- Added `public/.nojekyll` — without it, GitHub Pages' Jekyll processor can silently delete
  the `_next` folder (which holds all your CSS/JS), since Jekyll ignores underscore-prefixed
  folders by default.

**To redeploy:** run `npm run build` locally — this now produces an `out/` folder instead of
`.next`. That `out/` folder's contents are what needs to be live on GitHub Pages (via your
existing GitHub Actions workflow, or by pushing `out/`'s contents to a `gh-pages` branch,
whichever you're using). If you're not sure which method you're using, tell me and I can walk
through it.
